...
sce.TME$clusters <- droplevels(sce.TME$clusters)
colLabels(sce.TME) <- droplevels(colLabels(sce.TME) )

  
#filter unexpressed genes
keep_feature <- rowSums(counts(sce.TME) > 5) > 10
  #
sum(keep_feature) # 5239
sce.TME <- sce.TME[keep_feature,]
rowData(sce.TME)$hugo.symbols <- rownames(sce.TME) %>% convert_mouse_to_human_symbols %>%
    convert_alias_to_symbols( "human", verbose = FALSE)
  
expressed_genes_sender <- rowData(sce.TME)$hugo.symbols %>% na.omit %>% unique   # 2565
  #expressed_genes_sender <- convert_alias_to_symbols(expressed_genes_sender, "human", verbose = FALSE)
  
sce.TME <- sce.TME[!is.na(rowData(sce.TME)$hugo.symbols),]
...
write.csv(ligand_activities %>% top_n(20, aupr_corrected) %>% arrange(-aupr_corrected), paste0("top20_predictedLigandsMDA_combinedTME.csv"))
  ...
  p_hist_lig_activity <- ggplot(ligand_activities, aes(x=aupr_corrected)) + 
    ...
    labs(x="ligand activity TME", y = "# ligands") +
    theme_classic()
  
  
  pdf(paste0("figures/MDA_ligands_histogram_combinedTME_on_mets.pdf"), width = 5, height = 5)
  p_hist_lig_activity + ggtitle(paste("HCC cancer cells affected by TME"))
  dev.off()
  

  
  sce.sel <- sce.TME[rowData(sce.TME)$hugo.symbols %in% best_upstream_ligands,]
  ...
  pdf(paste0("figures/MDA_dotplot_combinedTME.pdf"), width = 7, height = 4)
  ...
  p_ligand_target_network <- vis_ligand_target %>% make_heatmap_ggplot(paste0("Prioritized TME-ligands"),"differentially expressed genes in malignant cells", color = "purple",legend_position = "top", x_axis_position = "top",legend_title = "Regulatory potential") + scale_fill_gradient2(low = "whitesmoke",  high = "purple", breaks = c(0,0.1,0.2)) + theme(axis.text.x = element_text(face = "italic"))
  
  
  pdf(paste0("figures/MDA_target_regulatory_potential_TME_on_mets.pdf"), width = 8, height = 5)
  ...
  p_ligand_aupr  <- vis_ligand_aupr %>% make_heatmap_ggplot(paste0("Prioritized combined TME-ligands"),"Ligand activity", color = "darkorange",legend_position = "top", x_axis_position = "top", legend_title = "AUPR\n(target gene prediction ability)")
  #p_ligand_aupr
  ...
  p_ligand_receptor_network = vis_ligand_receptor_network %>% t() %>% make_heatmap_ggplot(paste("Prioritized combined TME-ligands"),"Receptors expressed by malignant cells", color = "mediumvioletred", x_axis_position = "top",legend_title = "Prior interaction potential")
  
  
  pdf(paste0("figures/MDA_ligand_receptor_regulatory_potential_combinedTME_on_mets.pdf"), width = 8, height = 5)
  ...
  cluster <- colLabels(sce.TME)
  condition <- sce.TME$cell.line
  location <- str_remove(sce.TME$SampleName,"_.")
  ...
  pdf(paste0("figures/MDA_Barplot_location_combinedTME.pdf"), width = 8, height = 4.5)
   print(barplot.location)
  dev.off()
  
  #TSNES + velocity
  dec.cell.type <- modelGeneVar(sce.TME)
  top.hvgs.cell.type <- getTopHVGs(dec.cell.type, n=2000)
  velo.out.cell.type <- scvelo(sce.TME, subset.row=top.hvgs.cell.type, assay.X="spliced")
  
  sce.TME$velocity_pseudotime <- velo.out.cell.type$velocity_pseudotime
  embedded.TSNE <- embedVelocity(reducedDim(sce.TME, "TSNE"), velo.out.cell.type)
  grid.df <- gridVectors(sce.TME, embedded.TSNE, use.dimred = "TSNE")
  
  colData(sce.TME)$location <- location 
  tsne1 <- plotReducedDim(sce.TME[,sce.TME$cell.line == "MDA231"], "TSNE", colour_by="clusters", text_colour = "black", text_by="clusters",text_size = 4,add_legend = FALSE) +ggtitle("MDA231")
  tsne.velo <- plotTSNE(sce.TME[,sce.TME$cell.line == "MDA231"], colour_by="velocity_pseudotime") +
  ...
  tsne.loc <- plotReducedDim(sce.TME[,sce.TME$cell.line == "MDA231"], "TSNE", colour_by="location", text_colour = "black", text_by="location",text_size = 4,add_legend = FALSE) +ggtitle("MDA231")
  ...
  figures_tsnes <- cowplot::plot_grid(
    ...
    tsne.loc + theme( legend.position = "top", axis.ticks = element_blank()) + theme(axis.title.x = element_text()) + ylab(""),
    align = "h",
    nrow = 1,
    rel_widths = c(1, 0.8))
  #print(figures_tsnes)
  ...
  pdf(paste0("figures/CombinedNicheNet_combinedTME_MDA231.pdf"), width = 20, height = 16)
  print(combined_plot)
  dev.off()
